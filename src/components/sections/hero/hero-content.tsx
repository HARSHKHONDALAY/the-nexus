...
export default function HeroContent({
  ...
  // Only render if event data exists from backend
        ...
        <div className="grid w-full gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(320px,0.58fr)] lg:items-end">
          <div className="max-w-[56rem]">
            ...
            <p className="mt-6 max-w-[38rem] text-[1.02rem] leading-8 text-white/68 sm:text-[1.08rem] md:mt-7 md:text-[1.14rem]">
              The Nexus is a Mumbai-born cultural ecosystem of chess rooms,
              art tables, workshops, and intentional nights where strangers
              slowly become familiar faces.
            </p>

            <div className="mt-8">
              <p className="text-[0.96rem] leading-7 text-white/54">
                No events are currently scheduled. Check back soon for upcoming experiences.
              </p>
            </div>
          </div>
          ...
          <aside className="hidden justify-self-end rounded-[2rem] border border-white/12 bg-black/28 p-5 text-left shadow-[0_30px_90px_rgba(0,0,0,0.25)] backdrop-blur-2xl lg:block">
            ...
            <p className="mt-4 max-w-[28ch] text-[0.96rem] leading-7 text-lime-100/58">
              Events will be announced soon. Join our community to stay updated.
            </p>
          </aside>
        </div>
      </div>
  ...
  const eventDate = event.dateLabel || "Date TBD";
  const eventPrice = event.price || "Price TBD";
    ...
    <div className="container-custom relative z-10 flex min-h-[100svh] items-end pb-24 pt-32 md:items-center md:pb-18 md:pt-36">
      <div className="grid w-full gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(320px,0.58fr)] lg:items-end">
        ...
        <aside className="hidden justify-self-end rounded-[2rem] border border-white/12 bg-black/28 p-5 text-left shadow-[0_30px_90px_rgba(0,0,0,0.25)] backdrop-blur-2xl lg:block">
          ...
          <p className="mt-4 max-w-[28ch] text-[0.96rem] leading-7 text-lime-100/58">
            22 seats remain in a room built for conversation, taste,
            and easy belonging.
          </p>
        </aside>
      </div>
